...
import {genericFacetResponseSelector} from '../../../features/facets/facet-set/facet-set-selectors';
...
>(engine: Engine, props: RangeFacetProps<T>) {
  ...
  return {
    ...
    get state() {
      ...
      const response = genericFacetResponseSelector(engine.state, facetId) as
        | R
        | undefined;
