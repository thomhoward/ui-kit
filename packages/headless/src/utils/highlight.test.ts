import {HighlightKeyword, highlightString} from './highlight';
...
describe('highlight', () => {
  ...
  it('should wrap the passed highlights with tags using the specified class name', () => {
    ...
    expect(highlightString(lorem, highlights, '<span>', '</span>')).toBe(
      expectedHighlight
    );
  });
  ...
  it('should return the string unchanged if "content" is an empty string', () => {
    ...
    expect(highlightString('', highlights, '<span>', '</span>')).toBe(
      expectedString
    );
  });

  it('should throw if "tag" is an empty string', () => {
    expect(() => highlightString(lorem, highlights, '', '</span>')).toThrow(
      'delimiters should be a non-empty string'
    );
  });
});
